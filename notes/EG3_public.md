...
In-paper anchor: `paper/BOMBIERI_LANG_CONJECTURE_PREPRINT.md` (`BL_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving sparsity of rational points on varieties of general type through an admissible rational-point closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`BL_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_BL`.
...
If near-failure extraction and no-Zeno spacing hold, then `BL_G3` prevents hidden drift before rigidity is applied.
