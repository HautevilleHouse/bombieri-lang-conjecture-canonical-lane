...
In-paper anchor: `paper/BOMBIERI_LANG_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving sparsity of rational points on varieties of general type through an admissible rational-point closure architecture`.
...
2. the endpoint-transfer lock `lang_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_BL` remains positive.
...
Payload: verify `lang_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_BL` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `lang_transfer`
